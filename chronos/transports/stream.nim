...
proc setupStreamTransportTracker(): StreamTransportTracker {.gcsafe.} =
  let res = StreamTransportTracker(opened: 0, closed: 0,
    dump: dumpTransportTracking, isLeaked: leakTransport)
  addTracker(StreamTransportTrackerName, res)
  res

proc setupStreamServerTracker(): StreamServerTracker {.gcsafe.} =
  let res = StreamServerTracker(opened: 0, closed: 0,
    dump: dumpServerTracking, isLeaked: leakServer)
...
when defined(nimdoc):
  ...
  proc isConnResetError(err: OSErrorCode): bool {.inline.} =
    (err == OSErrorCode(osdefs.WSAECONNRESET)) or
  ...
  proc connect*(address: TransportAddress,
    ...
    if address.family in {AddressFamily.IPv4, AddressFamily.IPv6}:
      ...
      var pipeHandle = osdefs.INVALID_HANDLE_VALUE
    ...
    while true:
      if server.apending:
        ...
        if server.status notin {ServerStatus.Stopped, ServerStatus.Closed}:
          ...
          if wres.isErr():
            raiseAssert osErrorMsg(wres.error())
          ...
          if res == 0:
            ...
            if int32(err) == osdefs.ERROR_OPERATION_ABORTED:
              ...
              discard
            elif int32(err) == osdefs.ERROR_PIPE_CONNECTED:
              discard
            else:
  ...
  proc accept*(server: StreamServer): Future[StreamTransport] =
        ...
        if server.status in {ServerStatus.Stopped, ServerStatus.Closed}:
          ...
          of OSErrorCode(-1):
            if setsockopt(SocketHandle(server.asock), cint(osdefs.SOL_SOCKET),
              ...
              let err = OSErrorCode(wsaGetLastError())
              server.asock.closeSocket()
              if int32(err) == osdefs.WSAENOTSOCK:
      ...
      if not(retFuture.finished()):
        if server.status in {ServerStatus.Stopped, ServerStatus.Closed}:
          ...
          if ovl.data.errCode == OSErrorCode(-1):
            ...
            server.sock =
              block:
                let res = server.createAcceptPipe()
                if res.isErr():
                  server.sock = asyncInvalidSocket
                  server.errorCode = res.error()
                  retFuture.fail(getTransportOsError(res.error()))
                  return
                res.get()
            trackStream(ntransp)
    ...
    if server.local.family in {AddressFamily.IPv4, AddressFamily.IPv6}:
      ...
      if server.asock == asyncInvalidSocket:
        let err = wsaGetLastError()
        case int(err)
        of osdefs.ERROR_TOO_MANY_OPEN_FILES, osdefs.WSAENOBUFS,
           osdefs.WSAEMFILE:
  ...
  proc readStreamLoop(udata: pointer) =
    ...
    if ReadClosed in transp.state:
      ...
      if transp.kind == TransportKind.Socket:
        let res = handleEintr(osdefs.recv(fd, addr transp.buffer[transp.offset],
                                          len(transp.buffer) - transp.offset,
                                          cint(0)))
        if res < 0:
          let err = osLastError()
          if int(err) in {ECONNRESET}:
            transp.state.incl({ReadEof, ReadPaused})
        ...
        let res = handleEintr(osdefs.read(cint(fd),
                                          addr transp.buffer[transp.offset],
                                          len(transp.buffer) - transp.offset))
        if res < 0:
          let err = osLastError()
          transp.state.incl(ReadPaused)
...
proc start2*(server: StreamServer): Result[void, OSErrorCode] =
  ## Starts ``server``.
  doAssert(not(isNil(server.function)), "You should not start server, " &
           "if you have not set processing callback!")
  ...
  while true:
    ...
    if done:
      ...
      if res.isErr():
        ...
        when defined(windows):
          # This assertion should could be changed, because at this moment
